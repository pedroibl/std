...
/**
 * Run cn's plugin-envelope contract against the target vault BEFORE building (AC4) — declared ≠ decorative.
 *
 ...
 *
 * ONCE PER INVOCATION, never per rebuild: it is called from `runCnDeploy` ahead of the first build, so
...
export function preflightVault(vault: string): string[] {
  ...
  try {
    ...
    // Re-badge so the caller's single `CnDeployError` catch keeps the 0/1/2 contract. The guards are
    // the same guards `resolveTarget` already ran, plus this command's own community-plugins.json ones.
    throw e instanceof CnVerifyError ? new CnDeployError(e.message) : e;
  }
...
export async function runCnDeploy(argv: string[], deps: CnDeployDeps = {}): Promise<number> {
  ...
  // 7.3's PREFLIGHT (AC4). Runs after resolveTarget's guards and before the build — so an `error`
  // envelope aborts having written nothing, and (with --watch) the watcher never registers. Exactly
  // once per invocation, never per rebuild.
  try {
    for (const line of preflightVault(vault as string)) log(line);
  } catch (e) {
