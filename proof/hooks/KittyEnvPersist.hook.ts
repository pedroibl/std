 ...
 *
 * Story 13.7 (AC8): the top-level stays SYNCHRONOUS. The stdin read keeps its sync `readFileSync(0)`
 * deliberately (adopting the async readStdinJson would force this synchronous top-level async for no
 * benefit); only the kitty-env write moves onto fsx.atomicWrite (torn-write-proof, same bytes, ensures
 * the dir itself). setTabState/readTabState/persistKittySession are the sync (unchanged) tab-setter API.
 */
...
// fail-OPEN: empty/malformed stdin → best-effort, proceed (SessionStart never blocks).
let sessionId = '';
let source = '';
try {
