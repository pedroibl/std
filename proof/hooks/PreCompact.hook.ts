 ...
 *
 * Story 13.7: stdin → stdio.readStdinJson; JSON/text reads → fsx.loadJson / readIfExists; the state-dir
 * scan keeps readdirSync (flat, no fsx primitive). The 13.5 isa-utils findArtifactPath import is left
 * BYTE-STABLE — 13.5 owns that collapse.
 */
...
function readJSON(path: string): any {
  // loadJson softens missing + corrupt → the fallback (faithful to the old existsSync+try/catch → null).
  return loadJson<any>(path, null);
}
...
async function main() {
  // Parse stdin. fail-OPEN (AD-9.4 Rule 2): null (empty/malformed/timeout) → empty input, continue.
  const input: HookInput = (await readStdinJson<HookInput>(3000)) ?? {};
