 ...
 *
 * Story 13.8 rewrite (context & prompt lifecycle cluster — WIRED UserPromptSubmit, async:true, HIGHEST BLAST):
 *   - P1: readStdinWithTimeout(5000)+JSON.parse → std/stdio.readStdinJson<HookInput>(5000); fail-OPEN
 *     `if(!data) process.exit(0)` (null on empty/malformed/timeout — this also removes the JSON.parse('')
 *     throw path that previously crashed via the kvPush-broken catch). Cite src/stdio/read.ts:7-12.
 *   - core.parseNdjson — getCustomTitle + getRecentContext per-line JSON.parse loops (entry SHAPE + last-wins/
 *     filter stay caller-local).
 *   - http.fetchWithTimeout — the Pulse-notify fetch (raw Response, fire-and-forget). ⚠ pass {timeout:5000},
 *     NOT signal: fetchWithTimeout OWNS/strips any caller signal and installs its own from opts.timeout. The
 *     surrounding try/catch is kept so a Pulse-down never breaks a prompt. URL + voice_id stay caller-local.
 *   - core.collapse — :354 (sanitizePromptForNaming tail) + :926 (cleanPrompt): clean `X.replace(/\s+/g,' ')
 *     .trim()` ≡ collapse(X). DEFER :190 quickTitle (validator N3: prompt.trim()…slice(0,200) has NO trailing
 *     trim; collapse adds one → byte-divergent → kept caller-local). Bare `.slice()` are NOT truncate.
 *   - DEFER fileLock (acquireLock :266-283, mkdir-mutex + stale-eviction) — 1 consumer → kept caller-local (D2).
 *   - PRESERVE the 7 reachable exit-0 paths + the kvPush try-scope (NOT hoisted). See main()'s outer catch.
 *     Frozen imports (isa-utils/paths/identity/tab-setter/output-validators/observability-transport/Inference)
 *     left byte-stable.
 */
...
import { parseNdjson, collapse } from 'std/core';
...
// ── Stdin Reader ──
// Story 13.8: readStdinWithTimeout removed — main() now uses std/stdio.readStdinJson (folds the
// read + JSON.parse + timeout into one posture-neutral primitive; null → exit 0 fail-open).
...
function sanitizePromptForNaming(prompt: string): string {
  // core.collapse ≡ the trailing `.replace(/\s+/g,' ').trim()` (clean 1:1) — wraps the replace-chain result.
  return collapse(stripPastedLetter(prompt)
...
function getCustomTitle(sessionId: string): string | null {
  ...
  try {
    ...
    // core.parseNdjson: per-line JSON.parse skipping blanks/failures — faithful to the prior loop.
    // The custom-title entry shape + last-wins logic stay caller-local.
    for (const e of parseNdjson<any>(readFileSync(jsonlPath, 'utf-8'))) {
...
function getRecentContext(transcriptPath: string, maxTurns: number = 6, includeAssistant: boolean = false): string {
  try {
    ...
    // core.parseNdjson: split on \n, skip blank, JSON.parse each (skip failures) — faithful to the prior
    // content.trim().split('\n') + per-line try/parse-skip. The entry-shape (type/message.content) filter
    // + text extraction stay caller-local.
    for (const entry of parseNdjson<any>(content)) {
...
async function main() {
  try {
    ...
    // P1 fail-OPEN (AD-9.4 Rule 2): readStdinJson folds read + JSON.parse + timeout into one primitive;
    // null (empty/malformed/timeout) → exit 0. This also removes the JSON.parse('') throw path that
    // previously crashed via the block-scoped-kvPush catch. 5000 = the prior readStdinWithTimeout window.
    ...
    // core.collapse ≡ .replace(/\s+/g,' ').trim() (clean 1:1); bare .slice(0,1000) is NOT truncate (no ellipsis).
    const cleanPrompt = collapse(prompt.replace(/<[^>]*>/g, ' ')).slice(0, 1000);
    ...
    try {
      ...
      if (result.success && result.parsed) {
        ...
        if (voiceContent) {
          ...
          try {
            // http.fetchWithTimeout: raw Response, fire-and-forget. ⚠ {timeout:5000} NOT signal — it OWNS
            // and installs its own AbortSignal (strips any caller signal). The try/catch is kept so a
            // Pulse-down (AbortError/network) never breaks a prompt. URL + voice_id stay caller-local.
    ...
    // ⚠ kvPush is intentionally kept BLOCK-SCOPED to the try (AC1 — NOT hoisted; scope not widened /
    // "not worsened"). The live hook's `await kvPush` HERE referenced an out-of-scope const → a latent
    // TS2304 / runtime ReferenceError that never reached exit(0) (it crashed first; the path was already
    // unreachable, and its one trigger — empty-stdin JSON.parse('') — is now removed by P1). To keep
    // typecheck:proof green WITHOUT hoisting kvPush (the prohibited "fix"), the unreachable-broken await
    // is dropped and this fatal path simply exits 0 (fail-open — the invariant "every reachable path
    // exits 0"). No reachable behavior changes; the seven in-try exit-0 paths are untouched.
    process.exit(0);
  }
}
