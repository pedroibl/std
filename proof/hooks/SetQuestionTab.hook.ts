...
interface QuestionHookInput {
  session_id?: string;
  tool_input?: { questions?: Array<{ header?: string; question?: string }> };
}
...
async function main() {
  // fail-OPEN (AD-9.4 Rule 2): null (empty/malformed/timeout stdin) → keep the fallback title + no
  // session id, then proceed non-blocking (the tab is still set to a generic "awaiting input" state).
  const input = await readStdinJson<QuestionHookInput>(2000);
