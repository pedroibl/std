 ...
 *
 * Story 13.8 rewrite (context & prompt lifecycle cluster — WIRED SessionStart, no stdin):
 *   - core.getMetaField — opinion **Confidence:** read (:104-105) → getMetaField(block,'Confidence') + parseFloat.
 *   - core.truncate — title clamp (:294) → truncate(title,60) (57+'...' output byte-identical).
 *   - core.isoDate — the toISOString().split('T')[0]/slice(0,7) date stamps (:126/127/347). UTC, VERBATIM.
 *     DECISION (recorded deferred-work §13-8): default isoDate UTC — the RELATIONSHIP note filenames are
 *     UTC-derived today; dateParts('Australia/Melbourne') would change which note matches. No dateParts.
 *   - fsx.loadJson — session-names.json (:189-195) + *-progress.json (:336). NOTE (deferred-work §13-8):
 *     loadJson re-throws a REAL fs fault where the :189-195 all-swallow kept {} → now propagates to main's
 *     outer catch (:515) which still exit(0) (net posture preserved); the :336 loadJson is inside a per-file
 *     try/catch so no propagation there.
 *   - DEFER core.parseFrontmatter (:233/:244) — /^status:\s*"?(\w+)"?/m captures only `\w+`; \w EXCLUDES `-`,
 *     so `status: in-progress` → live captures "in" while parseFrontmatter returns "in-progress" (validator
 *     E2). Different parsed status for the PAI vocab → kept per-field regex caller-local.
 *   - DEFER findSection (:100) — `content.split(/^### /gm)` is a ###-block splitter, NOT core.findSection
 *     (##-based, single-section). Kept caller-local.
 *   - DEFER report.lines() — no verbatim push/join('\n') site here (every builder depends on .length/.slice,
 *     and learningParts joins with '\n\n'); forcing it risks byte-drift on the injected context. Recorded.
 *   - FROZEN imports: ./lib/paths, ./lib/notifications (recordSessionStart), ./lib/learning-readback,
 *     ./lib/isa-utils (findArtifactPath) — all byte-stable. main() stays async; every path exits 0.
 */

import { readFileSync, existsSync, readdirSync } from 'fs';
import { join } from 'path';
import { getMetaField, isoDate, truncate } from 'std/core';
import { loadJson } from 'std/fsx';
import { getPaiDir, getSettingsPath } from './lib/paths';
import { recordSessionStart } from './lib/notifications';
import { loadLearningDigest, loadWisdomFrames, loadFailurePatterns, loadSignalTrends, loadSynthesisPatterns, loadRecentWins } from './lib/learning-readback';
...
function loadRelationshipContext(paiDir: string): string | null {
  ...
  if (existsSync(opinionsPath)) {
    try {
      ...
      // DEFER findSection: `split(/^### /gm)` is a ###-block splitter (not core.findSection). Kept local.
      const opinionBlocks = content.split(/^### /gm).slice(1);
      for (const block of opinionBlocks) {
        const lines = block.split('\n');
        const statement = lines[0]?.trim();
        // core.getMetaField reads the `**Confidence:**` bold-meta value (remainder-of-line); parseFloat
        // tolerates any trailing text, so this is behavior-equivalent to the prior `([\d.]+)` capture.
        const confRaw = getMetaField(block, 'Confidence');
        const confidence = confRaw ? parseFloat(confRaw) : 0;
  ...
  // core.isoDate = UTC toISOString().slice(0,10) — byte-identical to the prior .split('T')[0]. UTC default
  // (NOT dateParts(tz)): these note filenames are UTC-derived today; a Melbourne-local switch would change
  // which note matches. Decision recorded in deferred-work §13-8.
  const formatDate = (d: Date) => isoDate(d);
  const formatMonth = (d: Date) => isoDate(d).slice(0, 7);
...
function getRecentWorkSessions(paiDir: string): WorkSession[] {
  ...
  // fsx.loadJson: missing/corrupt → {} (faithful to the prior existsSync+parse+swallow). A REAL fs fault
  // (EACCES/ENOTDIR) now re-throws → propagates to main's outer catch (:515) which still exit(0) — net
  // posture preserved (non-blocking); the changed real-fault path is recorded in deferred-work §13-8.
  const namesPath = join(paiDir, 'MEMORY', 'STATE', 'session-names.json');
  const sessionNames = loadJson<Record<string, string>>(namesPath, {});
  ...
  try {
    ...
    for (const dirName of allDirs) {
      ...
      if (isaPath) {
        ...
        // DEFER parseFrontmatter (validator E2): /^status:\s*"?(\w+)"?/m captures only `\w+` and \w
        // EXCLUDES `-`, so `status: in-progress` → "in" — but parseFrontmatter would return the full
        // "in-progress", changing the parsed status. Kept the per-field regex caller-local.
        try {
        ...
        // Legacy: Read from META.yaml (per-field regex kept caller-local — same DEFER as above).
        try {
...
function getProjectProgress(paiDir: string): WorkSession[] {
  ...
  try {
    ...
    for (const file of files) {
      try {
        // fsx.loadJson: missing/corrupt → null (this is inside the per-file try/catch, so a REAL fs
        // fault throws → caught → skip, exactly as the prior readFileSync+JSON.parse+catch did).
        const progress = loadJson<ProgressFile | null>(join(progressDir, file), null);
        ...
        sessions.push({
          ...
          status: 'active',
          timestamp: isoDate(new Date(progress.updated)),
          stale: isStale,
...
async function main() {
  try {
    ...
    if (isDynamicEnabled(settings, 'learningReadback')) {
      const recentWins = loadRecentWins(paiDir);
      const learningDigest = loadLearningDigest(paiDir);
      const wisdomFrames = loadWisdomFrames(paiDir);
      const failurePatterns = loadFailurePatterns(paiDir, 3);
      const signalTrends = loadSignalTrends(paiDir);
      const synthesisPatterns = loadSynthesisPatterns(paiDir, 3);
      if (synthesisPatterns) {
        console.error(`🧭 Loaded synthesis patterns (${synthesisPatterns.length} chars)`);
      }

      const learningParts: string[] = [];
      // Encouragement first: wins → wisdom → signals lead so a session opens with what
      // is working, not a wall of complaints (PAI SessionStart discouragement fix).
      if (recentWins) learningParts.push(recentWins);
      ...
      // Watch-outs tail (capped to 3 each): corrections as a footnote, not the headline.
