...
import { loadJson, saveJson } from 'std/fsx';
...
function readStarts(): Record<string, StartRecord> {
  // loadJson softens missing + corrupt → {} (faithful to the old existsSync+try/catch reset).
  return loadJson<Record<string, StartRecord>>(STARTS_FILE, {});
}

function writeStarts(starts: Record<string, StartRecord>) {
  saveJson(STARTS_FILE, starts);
}
...
async function main() {
  // fail-OPEN (AD-9.4 Rule 2): null (empty/malformed/timeout stdin) → exit 0.
  const data = await readStdinJson<ToolHookInput>(2000);
  ...
  try {
    ...
    if (!isPost) {
      ...
      appendJsonlEvent(OBS_DIR, 'subagent-events.jsonl', event);
