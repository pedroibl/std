 ...
 *
 * Story 13.7: stdin → stdio.readStdinJson; filesystem reads/writes → fsx (readIfExists / atomicWrite /
 * exists), unlinkSync kept (no fsx primitive). The 13.5 isa-utils imports (readRegistry / writeRegistry /
 * findArtifactPath) are left BYTE-STABLE — 13.5 owns that collapse.
 */
...
function clearSessionWork(sessionId?: string): void {
  try {
    ...
    if (currentWork.session_dir) {
      ...
      if (isaPath) {
        const isaRaw = readIfExists(isaPath);
        if (isaRaw !== null) {
          let isaContent = isaRaw;
          ...
          atomicWrite(isaPath, isaContent);
          marked = true;
        }
      }
      ...
      if (metaRaw !== null) {
        ...
        atomicWrite(metaPath, metaContent);
        marked = true;
      }
    ...
    if (sessionId || currentWork.session_id) {
      ...
      try {
        const snRaw = readIfExists(snPath);
        if (snRaw !== null) {
          const names = JSON.parse(snRaw);
          if (names[sid]) {
            delete names[sid];
            atomicWrite(snPath, JSON.stringify(names, null, 2));
...
async function main() {
  try {
    ...
    // fail-OPEN (AD-9.4 Rule 2): null stdin → undefined session id, continue.
    const input = await readStdinJson<{ session_id?: string }>(3000);
