// ── Story 13.6 rewrite (security cluster) — two swaps, everything else verbatim:
//    - config load: existsSync + readFileSync (:49-63) → std/fsx exists + readIfExists (keep parseYaml —
//      a hand-rolled YAML parse in a SECURITY matcher would be a regression). Fail-behaviour preserved:
//      missing file → null → PatternInspector.inspect returns deny('…fail-closed') (:200, the ONE existing
//      fail-closed site — PRESERVED, not regressed).
//    - matchesPathPattern metachar-escape (:97) → std/core escapeRegExp. EN4-verified BYTE-EQUIVALENT for
//      THIS matcher: escapeRegExp additionally escapes `*` and `?`; `*` is placeholder-protected (the
//      DOUBLESTAR/SINGLESTAR ASCII sentinels are substituted two lines up and escapeRegExp leaves them
//      untouched), and `?` does NOT appear in ANY `paths:` pattern in PATTERNS.yaml or Patterns.example.yaml
//      (grep-verified live 2026-07-16 — the `?` occurrences are all in the `bash:` table, which routes
//      through matchesBashPattern, never here). matchRules/globMatch stay caller-local (DEFER — see §13-6).
import { resolve } from 'path';
import { homedir } from 'os';
import { parse as parseYaml } from 'yaml';
import { exists, readIfExists } from 'std/fsx';
import { escapeRegExp } from 'std/core';
...
function loadPatterns(): PatternsConfig | null {
  ...
  if (exists(USER_PATTERNS_PATH)) {
    patternsPath = USER_PATTERNS_PATH;
  } else if (exists(SYSTEM_PATTERNS_PATH)) {
  ...
  try {
    const content = readIfExists(patternsPath);
