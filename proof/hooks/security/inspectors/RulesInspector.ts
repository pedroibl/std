 ...
 *
 * ── Story 13.6 rewrite (security cluster) — the SINGLE win is the fs → std/fsx swap in loadRules:
 *    - existsSync + readFileSync (:59-64) → fsx.readIfExists (ENOENT → null in one call; a non-ENOENT fs
 *      error re-throws, same as the old readFileSync throw). Behaviour preserved: missing/empty rules → null
 *      → inspect returns ALLOW (the DELIBERATE fail-OPEN — the LLM is unreliable, so a missing ruleset must
 *      not block; PRESERVED, not flipped to deny). The `inference` coupling stays a PAI tool (imported by the
 *      identical byte-verbatim `../../../PAI/TOOLS/Inference.ts` string — NOT routed through std/http); its
 *      hermetic test drives loadRules via a temp PAI_DIR and never invokes a real LLM (the proof Inference
 *      shim returns success:false).
 */
...
class RulesInspector implements Inspector {
  ...
  private loadRules(): string | null {
    ...
    const raw = readIfExists(RULES_PATH);
    if (raw === null) {
