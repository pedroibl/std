 ...
 *
 * Story 13.7 delamination (partial): the filesystem mechanisms moved onto tested `fsx` primitives —
 * per-window/session state writes → fsx.atomicWrite (torn-write-proof; same bytes), reads → fsx.readIfExists,
 * probes → fsx.exists. Kitty/cmux remote-control command STRINGS + flags stay caller-local (D4).
 *
 * DEFERRED (deferred-work §13-7): the execSync → proc.spawnCapture swap. spawnCapture is ASYNC, which would
 * force setTabState/setPhaseTab to Promise<void>; but they have DEPLOYED callers OUTSIDE this cluster —
 * ISASync.hook.ts (13.5-owned, frozen) and PromptProcessing.hook.ts (out of scope) — that call them
 * fire-and-forget WITHOUT await. An async signature would float those promises so the spawn may not complete
 * before the hook process exits = a runtime regression in hooks 13.7 must not modify. std has no synchronous
 * subprocess-capture primitive (substrate gap). execSync is retained here until that migration is coordinated.
 */
...
function getKittyEnv(sessionId?: string): { listenOn: string | null; windowId: string | null } {
  ...
  if (sessionId) {
    try {
      ...
      const rawEntry = readIfExists(sessionPath);
      if (rawEntry !== null) {
...
export function cleanupKittySession(sessionId: string): void {
  try {
    const sessionPath = join(KITTY_SESSIONS_DIR, `${sessionId}.json`);
    if (exists(sessionPath)) unlinkSync(sessionPath);
  } catch { /* silent */ }
}
...
function cleanupStaleStateFiles(): void {
  try {
    if (!exists(TAB_TITLES_DIR)) return;
    ...
    const defaultSocket = `/tmp/kitty-${process.env.USER}`;
    const socketPath = process.env.KITTY_LISTEN_ON || (exists(defaultSocket) ? `unix:${defaultSocket}` : null);
...
export function setTabState(opts: SetTabOptions): void {
  ...
  try {
    if (state === 'idle') {
      ...
      if (exists(statePath)) unlinkSync(statePath);
    } else {
      ...
      atomicWrite(join(TAB_TITLES_DIR, `${windowId}.json`), JSON.stringify(stateData));
    }
  } catch { /* silent */ }
...
export function readTabState(sessionId?: string): { title: string; state: TabState; previousTitle?: string; phase?: string } | null {
  ...
  try {
    const statePath = join(TAB_TITLES_DIR, `${windowId}.json`);
    const rawState = readIfExists(statePath);
...
export function getSessionOneWord(sessionId: string): string | null {
  try {
    ...
    const rawNames = readIfExists(namesPath);
...
export function setPhaseTab(phase: AlgorithmTabPhase, sessionId: string, summary?: string): void {
  ...
  try {
    atomicWrite(join(TAB_TITLES_DIR, `${windowId}.json`), JSON.stringify({
      title,
