...
//
// ── Story 13.5 rewrite (AD-9.4 Rule 3 — the isa-utils DEDICATED FINAL PASS) ──────────────────────────
// This is the Strangler-Fig collapse of the 946-LOC internals onto the tested std substrate, BEHIND A
// BYTE-STABLE FROZEN FACADE. 10 live importers across 5 stories (13.3/13.4/13.5/13.7/13.8) pin every
// exported signature below — the collapse touches ONLY function bodies; every `export` line is unchanged.
// PRIMITIVE SWAPS (internals only):
//   readRegistry     JSON.parse(readFileSync) + `.sessions` guard   → fsx.loadJson(+ the SAME guard re-applied)
//   writeRegistry    mkdirSync + writeFileSync(tmp) + renameSync     → fsx.ensureDir + fsx.atomicWrite
//   findArtifactPath existsSync probes                               → fsx.exists
//   findLatestISA    readdirSync + statSync(mtime) [SHALLOW]         → readdirSync + fsx.statMtime (NOT walkFiles — E4)
//   parseFrontmatter hand-rolled `/^---\n…/` + literal values        → core.parseFrontmatter + shape-adapt (AC2)
//   syncToWorkJson   session-names.json existsSync+readFileSync      → fsx.loadJson
// KEEP CALLER-LOCAL / DEFER (do NOT collapse — validators E1–E6, ENH-1/3):
//   extractCriteriaSection / extractIntentSnippet — multi-variant regex + `\n---` boundary; NOT core.findSection.
//   getSessionAgents — SYNC facade; proc.spawnCapture is async (would break the facade + 1 internal sync
//     caller). DEFER the proc swap; the inner execSync('tail -200 …') stays caller-local.
//   writeFrontmatterField — 0 live callers; keep verbatim. DEFER.
//   countCriteria / parseCriteriaList / diagnoseCriteria / parseCapabilities / appendPhase / the session
//     upsert+rating family — ISC-domain logic, caller-local identity (D4), byte-verbatim.

import { readdirSync } from 'fs';
import { join } from 'path';
import { paiPath } from './paths';
import { parseFrontmatter as coreParseFrontmatter } from 'std/core';
import { atomicWrite, ensureDir, exists, loadJson, statMtime } from 'std/fsx';
 ...
 * legacy PRD.md.
 *
 * SHALLOW one-level scan (readdirSync of WORK_DIR + per-dir findArtifactPath),
 * NOT a recursive fsx.walkFiles — a recursive walk would surface nested ISA.md
 * the one-level scan never sees, returning a different "latest" (validator E4).
 * mtime read goes through fsx.statMtime (fail-soft: a vanished/unstatable file
 * → 0 → sorts oldest, matching the old try/catch-skip contract).
 */
export function findLatestISA(): string | null {
  if (!exists(WORK_DIR)) return null;
  ...
  for (const dir of readdirSync(WORK_DIR)) {
    ...
    if (mtime > latestMtime) { latestMtime = mtime; latest = candidate; }
  }
  return latest;
}
...
export function parseFrontmatter(content: string): Record<string, string> | null {
  // core.parseFrontmatter (src/core/parse.ts:50) returns Record<string, string | string[]> and NEVER
  // null — it yields {} when there is no `---` block. The frozen facade returns Record<string,string> | null.
  // SHAPE-ADAPT (AC2):
  const parsed = coreParseFrontmatter(content);
  //  (a) empty {} (no `---` block) → null. The load-bearing adapt: both ISASync (:55-57) and
  //      CheckpointPerISC (:159-160) guard `if (!fm) return`.
  if (Object.keys(parsed).length === 0) return null;
  //  (b) string[] → string coercion. DEFENSIVE-ONLY (validator E6): no live ISA frontmatter uses `[...]`
  //      values (fields are scalar slug/phase/… — asserted in the test), so this branch is unreachable in
  //      practice. Reconstruct the bracket literal to stay as close to the old facade's `"[a, b]"` byte
  //      shape as possible for the (never-hit) array case.
  const fm: Record<string, string> = {};
  for (const [key, value] of Object.entries(parsed)) {
    fm[key] = Array.isArray(value) ? `[${value.join(', ')}]` : value;
  }
  return fm;
}
...
//
// KEEP CALLER-LOCAL VERBATIM (validator ENH-1): CRITERIA_HEADING_RE is a MULTI-variant case-insensitive
// alternation with a `\n##\s+(?!#)|\n---` boundary. core.findSection (single literal heading,
// same-or-shallower depth, no `---` stop) CANNOT reproduce it, and this feeds parseCriteriaList →
// CheckpointPerISC's commit decisions. Precedent: proof/isa-utils.ts (12.5) keeps it verbatim.
...
//
// KEEP CALLER-LOCAL (validator ENH-1): H2-priority loop + placeholder-italic strip + 280-char `…`
// truncate is not a clean core swap (1 internal caller syncToWorkJson:655, 0 external; feeds only the
// dashboard `intent` field).
 ...
 *
 * DEFER the proc swap (validator E3 / AC7): this is SYNCHRONOUS (returns AgentEntry[]);
 * proc.spawnCapture returns Promise<SpawnResult>. Async-ifying it would break the frozen facade AND its
 * 1 internal sync caller (syncToWorkJson:628). The inner execSync('tail -200 …') stays caller-local.
 */
...
export function readRegistry(): { sessions: Record<string, any> } {
  // fsx.loadJson softens missing-file + unparseable-JSON to the fallback and re-throws a real fs fault
  // (matching the old JSON.parse(readFileSync) inside try/catch → {sessions:{}}). Then RE-APPLY the
  // `.sessions ? data : {sessions:{}}` guard (validator E5): loadJson returns the RAW parsed value, so a
  // sessionless JSON (`{"foo":1}`) must still normalize to an empty registry.
  const data = loadJson<{ sessions?: Record<string, any> }>(WORK_JSON, { sessions: {} });
  return data.sessions ? (data as { sessions: Record<string, any> }) : { sessions: {} };
}

export function writeRegistry(reg: { sessions: Record<string, any> }): void {
  // fsx.ensureDir (idempotent mkdir -p) + fsx.atomicWrite (tmp-sibling + rename) — same torn-write-proof
  // tmp+rename the hand-rolled body did, now on the tested edge.
  ensureDir(join(paiPath('MEMORY'), 'STATE'));
  atomicWrite(WORK_JSON, JSON.stringify(reg, null, 2));
}
...
export function syncToWorkJson(fm: Record<string, string>, isaPath: string, content?: string, sessionId?: string): void {
  ...
  // fsx.loadJson softens missing/unparseable → {} (matching the old existsSync-guarded best-effort read).
  let sessionName = existing.sessionName || '';
  if (sessionId) {
    const namesPath = paiPath('MEMORY', 'STATE', 'session-names.json');
    const names = loadJson<Record<string, string>>(namesPath, {});
